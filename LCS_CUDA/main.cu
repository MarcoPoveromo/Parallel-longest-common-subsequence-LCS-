#include <iostream>
#include <bits/stdc++.h>
#include "LCS/LCSStrategy.cuh"
#include "LCS/Implementations/LCS_v1.cuh"
#include "LCS/Result.cuh"
#include "LCS/Implementations/LCS_v2.cuh"
...
void handleArgs(int argc, char *argv[]) {
    if (argc != 3) {
        cout << "Usage: ./lcs.out <lcsType> <filePath> \n";
        exit(0);
    }
...
void writeCsv(const Result& result) {
    std::ofstream file;
    file.open("Outputs/output.csv", std::ios_base::app);
    file << X.size() << "," << lcsType << "," << result.getLcs() << "," << result.getTimeElapsed() << "\n";
    file.close();
}
